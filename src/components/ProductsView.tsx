...
export const ProductsView: React.FC<ProductsViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanProduct: Partial<Product> = {
      ...editingProduct,
      preco_custo: typeof editingProduct.preco_custo === 'number' ? editingProduct.preco_custo : (parseFloat(String(editingProduct.preco_custo || '0')) || 0),
      preco_venda: typeof editingProduct.preco_venda === 'number' ? editingProduct.preco_venda : (parseFloat(String(editingProduct.preco_venda || '0')) || 0),
      estoque_atual: typeof editingProduct.estoque_atual === 'number' ? editingProduct.estoque_atual : (parseInt(String(editingProduct.estoque_atual || '0'), 10) || 0),
      estoque_minimo: typeof editingProduct.estoque_minimo === 'number' ? editingProduct.estoque_minimo : (parseInt(String(editingProduct.estoque_minimo || '5'), 10) || 5),
    };

    onSaveProduct(cleanProduct);
                  ...
                  <td colSpan={8} className="py-16 text-center">
                    <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                      ...
                      <p className="text-xs text-slate-500 mb-4 text-center">
                        {products.length === 0 
                          ? 'Cadastre o primeiro produto do seu estoque para começar a registrar movimentações e gerenciar suas vendas.'
                          : 'Tente alterar os termos da busca ou limpar o filtro de categorias.'}
                      </p>
                      {products.length === 0 && canEdit && (
                        <button
                          onClick={handleOpenAdd}
                          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold flex items-center gap-2 transition-colors shadow-xs"
                        >
                          <Plus className="w-4 h-4" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    placeholder="0.00"
                    value={editingProduct.preco_custo === undefined || editingProduct.preco_custo === null ? '' : editingProduct.preco_custo}
                    onFocus={(e) => {
                      if (editingProduct.preco_custo === 0) {
                        setEditingProduct({ ...editingProduct, preco_custo: undefined });
                      } else {
                        e.target.select();
                      }
                    }}
                    onChange={(e) => {
                      ...
                      setEditingProduct({
                        ...editingProduct,
                        preco_custo: val === '' ? undefined : parseFloat(val)
                      });
                    }}
                    className="w-full text-xs p-2.5 border border-slate-300 rounded-lg focus:outline-none focus:border-blue-500"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    placeholder="0.00"
                    value={editingProduct.preco_venda === undefined || editingProduct.preco_venda === null ? '' : editingProduct.preco_venda}
                    onFocus={(e) => {
                      if (editingProduct.preco_venda === 0) {
                        setEditingProduct({ ...editingProduct, preco_venda: undefined });
                      } else {
                        e.target.select();
                      }
                    }}
                    onChange={(e) => {
                      ...
                      setEditingProduct({
                        ...editingProduct,
                        preco_venda: val === '' ? undefined : parseFloat(val)
                      });
                    }}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    placeholder="0"
                    value={editingProduct.estoque_atual === undefined || editingProduct.estoque_atual === null ? '' : editingProduct.estoque_atual}
                    onFocus={(e) => {
                      if (editingProduct.estoque_atual === 0) {
                        setEditingProduct({ ...editingProduct, estoque_atual: undefined });
                      } else {
                        e.target.select();
                      }
                    }}
                    onChange={(e) => {
                      ...
                      setEditingProduct({
                        ...editingProduct,
                        estoque_atual: val === '' ? undefined : parseInt(val, 10)
                      });
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    placeholder="5"
                    value={editingProduct.estoque_minimo === undefined || editingProduct.estoque_minimo === null ? '' : editingProduct.estoque_minimo}
                    onFocus={(e) => {
                      if (editingProduct.estoque_minimo === 0) {
                        setEditingProduct({ ...editingProduct, estoque_minimo: undefined });
                      } else {
                        e.target.select();
                      }
                    }}
                    onChange={(e) => {
                      ...
                      setEditingProduct({
                        ...editingProduct,
                        estoque_minimo: val === '' ? undefined : parseInt(val, 10)
                      });
                    }}
                    className="w-full text-xs p-2.5 border border-slate-300 rounded-lg focus:outline-none focus:border-blue-500"
                  />
